                ...
                <div class="template-demo">
                    <nav aria-label="breadcrumb" class="nav-container">
                        <ol class="breadcrumb breadcrumb-custom ">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>Training Teacher</span></li>
                        </ol>
                              ...
                              @foreach($teachers as $teacher)
                                  <tr id="{{$teacher->id}}">
                                      ...
                                      <td>
                                        ...
                                        <input data-id="{{$teacher->id}}" class="toggle-class" type="checkbox" data-width="80px" data-size="xs" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-on="Complete" data-off="Incomplete" {{ $teacher->status ? 'checked' : '' }}>
                                      </td>
        ...
        <div class="row">
          <div class="col-12">
            <div class="table-responsive table table-bordered print-container p-3" id="div-id-name"><br>
                ...
                <h3  style="text-align:center">All Training Teacher's List</h3>
...
@push('custom-scripts')
  ...
  {{-- Ajax delete by modal js code start --}}
    ...
    <script type="text/javascript">
      ...
      cache: false,
      type: 'delete',
      url:"training-teacher/delete/"+user_id,
      dataType: "JSON",
      data: {
      "id": user_id
      ...
      window.location.replace('training-teacher');
      }
      })
      });
    </script>
  ...
  {{-- Ajax View by modal js code start --}}
      <script type="text/javascript">
        ...
        $('#status').text((data.teachers.status) ? "Training Complete" : "Training Incomplete");
        $('#password').text(data.teachers.user.raw_password); 
        $('#nid_no').text(data.teachers.nid_no);  
        $('#job_institution_name').text(data.teachers.job_institution_name);  
      ...
      $(function() {
        $('.toggle-class').change(function() {
            ...
            $.ajax({
                type: "GET",
                dataType: "json",
                url: 'training-teacher/changeTeacherTrainingStatus',
                data: {'status': status, 'category_id': category_id},
                success: function(data){
                  showSuccessToast('Tranning status changed successfully');
