                ...
                <div class="template-demo">
                    <nav aria-label="breadcrumb" class="nav-container">
                        <ol class="breadcrumb breadcrumb-custom ">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>Teacher Responsibility</span></li>
                        </ol>
                        @permission('add_teacher_btn')
                          <a href="{{ route('admin.teacher.teacher_responsibility_create') }}"
                                class="btn btn-sm btn-info button-custom">Add Teacher Responsibility
                          </a>
                        @endpermission
                    </nav>
                </div>
                        ...
                        <table id="order-listing" class="table">
                          ...
                          <tbody>
                              @php
                                  $key = 0;
                              @endphp
                              @foreach($responsibility as $teacher)
                                  <tr id="{{$teacher->id}}">
                                      <td>{{ ++$key }}</td>
                                      <td>{!! Str::of($teacher->description)->limit(90) ?? '-' !!}</td>
                                      <td>
                                      ...
                                      <td style="text-align: center">
                                        @permission('view_teacher')
                                          ...
                                          @permission('edit_teacher')
                                            <a href="{{ route('admin.teacher.teacher_responsibility_edit', $teacher->id)}}" class="btn btn-success"
                                              title="Edit">
        ...
        <div class="row">
          <div class="col-12">
            <div class="table-responsive table table-bordered print-container p-3" id="div-id-name"><br>
                ...
                <h3  style="text-align:center">Teacher Responsibility Content</h3>
                <table id="order-listing" class="table student-dm">
                  <thead>
                    <tr>
                      <th style="text-align:center; font-size: 20px;">SL#</th>
                      <th style="text-align:center; font-size: 20px;">Responsibility</th>
                    </tr>
                  </thead>
                  <tbody>
                    @php
                    ...
                    @foreach($responsibility as $teacher)
                        <tr id="{{$teacher->id}}">
                            ...
                            <td style="text-align:center;font-size:18px;">{{ $teacher->description ?? '-' }}</td>
                        </tr>
                    @endforeach
                  </tbody>
                </table><br>
