                ...
                <div class="template-demo">
                    <nav aria-label="breadcrumb" class="nav-container">
                        <ol class="breadcrumb breadcrumb-custom ">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>Cash On payment  List</span></li>
                        </ol>
                    </nav>
                </div>
            </div>
                          ...
                          <tbody>
                              ...
                              @foreach($list as $key=>$lists)
                                  <tr id="{{$lists->id}}">
                                      ...
                                      <td>{{ $lists->phone ?? '-' }}</td>
                                      <td>{{ $lists->email ?? '-' }}</td>
                                      <td style="text-align:center">{{ $lists->course->full_name ?? '-' }}</td>
                                      <td > <div style="width: 80px" class=" text-center">{{ $lists->batch->name ?? '-' }}</div> </td>
                                      <td style="text-align:center">{{ $lists->course_fee ?? '-' }}</td>
          ...
          <div class="col-12">
            <div class="print-container" id="div-id-name"><br>
              <div class="row justify-content-center align-items-center mb-3">
                ...
                <h3  style="text-align:center">Aplicant List</h3>
                  ...
                  <tbody>
                    @foreach($list as $key=>$lists)
                        <tr id="{{$lists->id}}">
                            ...
                            <td>{{ $lists->email ?? '-' }}</td>
                            <td style="text-align:center">{{ $lists->course->full_name ?? '-' }}</td>
                            <td style="text-align:center">{{ $lists->batch->name ?? '-' }}</td>
                            <td style="text-align:center">{{ $lists->course_fee ?? '-' }}</td>
                        </tr>
                    @endforeach
                  </tbody>
