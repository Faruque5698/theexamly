                ...
                <div class="template-demo">
                    <nav aria-label="breadcrumb" class="nav-container">
                        <ol class="breadcrumb breadcrumb-custom ">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>Promotional Modal</span></li>
                        </ol>
                        @permission('add_promotionalModal_btn') 
                            <a href="{{ route('admin.modal.create') }}" class="btn btn-sm btn-info button-custom">Add Promotional Modal
                            </a>
                        @endpermission
                    </nav>
                </div>
                            ...
                            <table id="order-listing" class="table text-center">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @permission('activate_status_promotional_modal') 
                                                <input data-id="{{$modal->id}}" class="toggle-class" type="checkbox" data-width="70px" data-size="xs" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-on="Publish" data-off="Unpublish" {{ $modal->status ? 'checked' : '' }}>
                                            @endpermission
                                        </td>
                                        <td>
                                            ...
                                            @permission('edit_promotional_modal')  
                                                <a href="{{ route("admin.modal.edit", $modal)}}" class="btn btn-success"
...
@include('backend.pages.modal.modal.modal-delete')
@include('backend.pages.modal.modal.modal-view')
@endsection
    ...
    $(document).on('click', '.view-modal', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#description').text(data.modal.description);
                    $('#status').text((data.modal.status) ? "Published" : "Unpublished");
                    $('#image'). attr("src", APP_URL+"/public/uploads/files/modal/"+data.modal.image);
