                ...
                <div class="template-demo">
                    <nav aria-label="breadcrumb" class="nav-container">
                        <ol class="breadcrumb breadcrumb-custom ">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>Feature</span></li>
                        </ol>
                        @permission('add_blog_btn') 
                            <a href="{{ route('admin.feature.create') }}" class="btn btn-sm btn-info button-custom">Add New
                                Feature
                            </a>
                        @endpermission
                    </nav>
                            ...
                            <table id="order-listing" class="table text-center">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ ++$key }}</td>
                                        <td>{{ $feature->title }}</td>
                                        <td class="word_breck">{!! Str::of($feature->description)->limit(350) !!}</td>
                                        <td>
                                            @permission('activate_notice') 
                                                <input data-id="{{$feature->id}}" class="toggle-class" type="checkbox" data-width="70px" data-size="xs" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-on="Publish" data-off="Unpublish" {{ $feature->status ? 'checked' : '' }}>
                                            @endpermission
                                        </td>
                                        <td>
                                            ...
                                            @permission('edit_blog')  
                                                <a href="{{ route("admin.feature.edit", $feature)}}" class="btn btn-success"
...
@include('backend.pages.features.modal.feature-delete')
@include('backend.pages.features.modal.feature-view')
@endsection
    ...
    $(document).on('click', '.view-modal', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#title').text(data.feature.title);
                    $('#description').html(data.feature.description);
                    $('#status').text((data.feature.status) ? "Published" : "Unpublished");
                    $('#image'). attr("src", APP_URL+"/public/uploads/files/features/"+data.feature.image);
