                ...
                <div class="template-demo">
                    <nav aria-label="breadcrumb" class="nav-container">
                        <ol class="breadcrumb breadcrumb-custom ">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>Zoom Api List</span></li>
                        </ol>
                        
                        @permission('add_staff')
                        <a href="{{ route('admin.communication.zoomCreate') }}"
                            class="btn btn-sm btn-info button-custom">Create New Zoom API
                        </a>
                        @endpermission
                        ...
                        <table id="order-listing" class="table">
                          <thead>
                            <tr>
                              ...
                              {{-- <th style="text-align:center">Zoom API Url</th> --}}
                              <th style="text-align:center">Zoom Api Key</th>
                              <th style="text-align:center">Zoom Api Secret key</th>
                          ...
                          <tbody>
                            ...
                            @foreach($users as $data)
                                <tr id="{{$data->id}}">
                                    ...
                                    {{-- <td>{{ $data->zoom_api_url ?? '-' }}</td> --}}
                                    <td>{{ $data->zoom_api_key ?? '-' }}</td>
                                    <td>{{ $data->zoom_api_secret ?? '-' }}</td>
                                    <td>
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-lg-right" aria-labelledby="dropdownMenuButton">
                                                @permission('edit_student')
                                                    ...
                                                    <a href="{{ route("admin.communication.editApiData", $data->id)}}" class="dropdown-item" title="Edit" role="button">
                ...
                <table id="order-listing" class="table student-dm">
                  <thead>
                    <tr>
                      ...
                      <th style="text-align:center; font-size: 20px;">Zoom API Url</th>
                      <th style="text-align:center; font-size: 20px;">Zoom Api Key</th>
                      <th style="text-align:center; font-size: 20px;">Zoom_api_secret</th>
                    </tr>
                  </thead>
                  <tbody>
                    @php
                    ...
                    @foreach($users as $data)
                        <tr id="{{$data->id}}">
                            ...
                            <td style="text-align:center;font-size:18px;">{{ $data->user->zoom_api_url ?? '-' }}</td>
                            <td style="text-align:center;font-size:18px;">{{ $data->user->zoom_api_key ?? '-' }}</td>
                            <td style="text-align:center;font-size:18px;">{{ $data->zoom_api_secret ?? '-' }}</td>
                        </tr>
                    @endforeach
                  </tbody>
                </table>
...
@push('custom-scripts')
    ...
    cache: false,
    type: 'delete',
    url:"deleteApiData/delete/"+user_id,
    dataType: "JSON",
    data: {
    "id": user_id
    ...
    window.location.replace('zoomIndex');
    }
    })
    });
    </script>
